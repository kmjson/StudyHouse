...
<script context="module">
  ...
  <script>
     ...
     */
    const baseIsTabbable = (/** @type {{ tabIndex: number; hidden: any; disabled: any; style: { display: string; }; type: string; offsetWidth: any; offsetHeight: any; getClientRects: () => { (): any; new (): any; length: any; }; }} */ node) => (
      node.tabIndex >= 0 && 
      !node.hidden && 
      !node.disabled && 
